...
namespace Infiniminer
{
    public class InfiniminerServer
    {
        ...
        string levelToLoad = "";
        string greeter = "";
        List<NetPeer> toGreet = new List<NetPeer>();
...
/*            if (varGetB("insanelava"))//insaneLava)
        ...
        static string GetPath(string filename) => Path.GetFullPath(filename, Platform.GetBasePath());
        ...
        public void SetBlock(ushort x, ushort y, ushort z, BlockType blockType, PlayerTeam team)
        {
            ...
            msgBuffer.Put((byte)InfiniminerMessage.BlockSet);
            msgBuffer.Put((byte)x);
            msgBuffer.Put((byte)y);
            msgBuffer.Put((byte)z);
            msgBuffer.Put((byte)blockType);
            foreach (var netConn in playerList.Keys)
                if (netConn.ConnectionState == ConnectionState.Connected)
                    netConn.Send(msgBuffer, 0, DeliveryMethod.ReliableOrdered);
        ...
        public bool Start()
        {
            ...
            DatafileWriter dataFile = new DatafileWriter(GetPath("server.config.txt"));
            ...
            {
                request.Data.TryGetString(out var appname);
                request.Data.TryGetString(out var playerName);
                playerName ??= "";
                playerName = Defines.Sanitize(playerName);
                if (playerName.Length == 0)
                    playerName = "Player";
                request.Data.TryGetString(out var clientVersion);
                if (appname != "INFINIMINER")
                {
                    request.RejectForce();
                }
                if (playerList.Count >= varGetI("maxplayers"))
                {
                    request.Reject("MAXPLAYERS;");
                }
                else if (clientVersion != Defines.INFINIMINER_VERSION)
                {
                    request.Reject("VER;" + Defines.INFINIMINER_VERSION);
                }
                else if (banList.Contains(request.RemoteEndPoint.Address.ToString()))
                {
                    request.Reject("BAN;");
                }
                else
                {
                    ...
                    Player newPlayer = new Player(peer, null);
                    newPlayer.Handle = playerName;
                    if (admins.ContainsKey(newPlayer.IP))
                        newPlayer.admin = admins[newPlayer.IP];
                    playerList[peer] = newPlayer;
                    //Check if we should compress the map for the client
                    try
                    {
                        bool compression = request.Data.GetBool();
                        if (compression)
                            playerList[peer].compression = true;
                    } catch { }
                    toGreet.Add(peer);
                    PublicServerListUpdate(true);
                    ConsoleWrite("CONNECT: " + playerList[peer].Handle + " ( " + playerList[peer].IP + " )");
                    SendCurrentMap(peer);
                    SendPlayerJoined(newPlayer);
                }
            };
            listener.PeerDisconnectedEvent += (peer, info) =>
            {
                if (!this.playerList.ContainsKey(peer))
                {
                    return;
                }
                Player player = playerList[peer];
                ConsoleWrite("DISCONNECT: " + playerList[peer].Handle);
                SendPlayerLeft(player, player.Kicked ? "WAS KICKED FROM THE GAME!" : "HAS ABANDONED THEIR DUTIES!");
                if (playerList.ContainsKey(peer))
            ...
            listener.NetworkReceiveUnconnectedEvent += (point, reader, type) =>
            {
                ...
                if (type == UnconnectedMessageType.Broadcast &&
                    reader.TryGetSpan(sig) && sig.SequenceEqual(Defines.AppSignature))
                {
                    var dw = new NetDataWriter();
                    for(int i = 0; i <sig.Length; i++)
                        dw.Put(sig[i]);
                    netServer.SendUnconnectedMessage(dw, point);
                }
            };
            listener.NetworkReceiveEvent += (msgSender, msgBuffer, channel, method) =>
            {
                ...
                switch (dataType)
                {
                    case InfiniminerMessage.ChatMessage:
                    {
                        ...
                        if (!ProcessCommand(chatString, GetAdmin(playerList[msgSender].IP), playerList[msgSender]))
                        {
                            ...
                            var chatPacket = new NetDataWriter();
                            chatPacket.Put((byte) InfiniminerMessage.ChatMessage);
                            chatPacket.Put((byte) ((player.Team == PlayerTeam.Red)
                                ? ChatMessageType.SayRedTeam
                                : ChatMessageType.SayBlueTeam));
                            chatPacket.Put(chatString);
                            ...
                            foreach (Player p in playerList.Values)
                            {
                                if (chatType == ChatMessageType.SayAll ||
                                    ...
                                    if (p.NetConn.ConnectionState == ConnectionState.Connected)
                                        p.NetConn.Send(chatPacket, 3, DeliveryMethod.ReliableOrdered);
                            }
                        }
                    }
                        break;
                    ...
                    {
                        Vector3 playerPosition = msgBuffer.GetVector3();
                        Vector3 playerHeading = msgBuffer.GetVector3();
                        PlayerTools playerTool = (PlayerTools) msgBuffer.GetByte();
                    ...
                    {
                        PlayerClass playerClass = (PlayerClass) msgBuffer.GetByte();
                    ...
                    {
                        PlayerTeam playerTeam = (PlayerTeam) msgBuffer.GetByte();
                    ...
                    case InfiniminerMessage.PlayerDead:
                    {
                        ...
                        string deathMessage = msgBuffer.GetString();
                        if (deathMessage != "")
                        {
                            var chatPacket = new NetDataWriter();
                            chatPacket.Put((byte) InfiniminerMessage.ChatMessage);
                            chatPacket.Put((byte) (player.Team == PlayerTeam.Red
                                ? ChatMessageType.SayRedTeam
                                : ChatMessageType.SayBlueTeam));
                            chatPacket.Put(player.Handle + " " + deathMessage);
                            foreach (var netConn in playerList.Keys)
                                if (netConn.ConnectionState == ConnectionState.Connected)
                                    netConn.Send(chatPacket, 3, DeliveryMethod.ReliableOrdered);
                    ...
                    case InfiniminerMessage.PlayerAlive:
                    {
                        if (toGreet.Contains(msgSender))
                        {
                            ...
                            if (greeting != "")
                            {
                                var greetBuffer = new NetDataWriter();
                                greetBuffer.Put((byte) InfiniminerMessage.ChatMessage);
                                greetBuffer.Put((byte) ChatMessageType.SayAll);
                                greetBuffer.Put(Defines.Sanitize(greeting));
                                msgSender.Send(greetBuffer, 3, DeliveryMethod.ReliableOrdered);
                            }
                    ...
                    {
                        player.Position = msgBuffer.GetVector3();
                    ...
                    {
                        SendPlayerPing((uint) msgBuffer.GetInt());
                    }
                        break;
            ...
            netServer = new NetManager(listener);
            netServer.IPv6Enabled = true;
            netServer.ChannelsCount = 4;
            netServer.BroadcastReceiveEnabled = true;
            netServer.Start(5565);
        ...
        {
            if (player.NetConn.ConnectionState != ConnectionState.Connected)
                return;

            // ore, cash, weight, max ore, max weight, team ore, red cash, blue cash, all uint
            var msgBuffer = new NetDataWriter();
            msgBuffer.Put((byte)InfiniminerMessage.TriggerConstructionGunAnimation);
            msgBuffer.Put(animationValue);
            player.NetConn.Send(msgBuffer, 1, DeliveryMethod.ReliableOrdered);
        }
        ...
        public void ExplosionEffectAtPoint(int x, int y, int z)
        {
            ...
            var msgBuffer = new NetDataWriter();
            msgBuffer.Put((byte)InfiniminerMessage.TriggerExplosion);
            msgBuffer.Put(new Vector3(x, y, z));
            foreach (var netConn in playerList.Keys)
                if (netConn.ConnectionState == ConnectionState.Connected)
                    netConn.Send(msgBuffer, 0, DeliveryMethod.ReliableUnordered);
        ...
        public void SendServerMessageToPlayer(string message, NetPeer conn)
        {
            if (conn.ConnectionState == ConnectionState.Connected)
            {
                var msgBuffer = new NetDataWriter();
                msgBuffer.Put((byte)InfiniminerMessage.ChatMessage);
                msgBuffer.Put((byte)ChatMessageType.SayAll);
                msgBuffer.Put(Defines.Sanitize(message));
                conn.Send(msgBuffer, 3, DeliveryMethod.ReliableOrdered);
            }
        }
        ...
        public void SendServerMessage(string message)
        {
            ...
            foreach (var netConn in playerList.Keys)
                if (netConn.ConnectionState == ConnectionState.Connected)
                    netConn.Send(msgBuffer, 3, DeliveryMethod.ReliableOrdered);
        }
        ...
        public void SendResourceUpdate(Player player)
        {
            ...
            // ore, cash, weight, max ore, max weight, team ore, red cash, blue cash, all uint
            var msgBuffer = new NetDataWriter();
            msgBuffer.Put((byte)InfiniminerMessage.ResourceUpdate);
            msgBuffer.Put((uint)player.Ore);
            msgBuffer.Put((uint)player.Cash);
            msgBuffer.Put((uint)player.Weight);
            msgBuffer.Put((uint)player.OreMax);
            msgBuffer.Put((uint)player.WeightMax);
            msgBuffer.Put((uint)(player.Team == PlayerTeam.Red ? teamOreRed : teamOreBlue));
            msgBuffer.Put((uint)teamCashRed);
            msgBuffer.Put((uint)teamCashBlue);
            player.NetConn.Send(msgBuffer, 1, DeliveryMethod.ReliableOrdered);        
        }
        ...
        public void SendCurrentMap(NetPeer client)
        {
            MapSender ms = new MapSender(client, this, MAPSIZE,playerList[client].compression);
        ...
        /*public void SendCurrentMapB(NetConnection client)
        {
            ...
            for (byte x = 0; x < MAPSIZE; x++)
                ...
                {
                    var msgBuffer = new NetDataWriter();
                    msgBuffer.Put((byte)InfiniminerMessage.BlockBulkTransfer);
                    msgBuffer.Put(x);
                    msgBuffer.Put(y);
                    for (byte dy=0; dy<16; dy++)
                        for (byte z = 0; z < MAPSIZE; z++)
                            msgBuffer.Put((byte)(blockList[x, y+dy, z]));
        ...
        {
            var msgBuffer = new NetDataWriter();
            msgBuffer.Put((byte)InfiniminerMessage.PlayerPing);
            msgBuffer.Put(playerId);

            foreach (NetPeer netConn in playerList.Keys)
                if (netConn.ConnectionState == ConnectionState.Connected)
                    netConn.Send(msgBuffer, 0, DeliveryMethod.ReliableUnordered);
        }
        ...
        {
            var msgBuffer = new NetDataWriter();
            msgBuffer.Put((byte)InfiniminerMessage.PlayerUpdate);
            msgBuffer.Put((uint)player.ID);
            msgBuffer.Put(player.Position);
            msgBuffer.Put(player.Heading);
            msgBuffer.Put((byte)player.Tool);
            ...
            if (player.QueueAnimationBreak)
            {
                ...
                msgBuffer.Put(player.UsingTool);

            msgBuffer.Put((ushort)player.Score / 100);

            foreach (var netConn in playerList.Keys)
                if (netConn.ConnectionState == ConnectionState.Connected)
                    netConn.Send(msgBuffer, 1, DeliveryMethod.Sequenced);
        }

        public void SendSetBeacon(Vector3 position, string text, PlayerTeam team)
        {
            var msgBuffer = new NetDataWriter();
            msgBuffer.Put((byte)InfiniminerMessage.SetBeacon);
            msgBuffer.Put(position);
            msgBuffer.Put(text);
            msgBuffer.Put((byte)team);
            foreach (var netConn in playerList.Keys)
                if (netConn.ConnectionState == ConnectionState.Connected)
                    netConn.Send(msgBuffer, 2, DeliveryMethod.ReliableOrdered);
        }
        ...
        public void SendPlayerJoined(Player player)
        {
            ...
            foreach (Player p in playerList.Values)
            {
                msgBuffer.Put((byte)InfiniminerMessage.PlayerJoined);
                msgBuffer.Put((uint)p.ID);
                msgBuffer.Put(p.Handle);
                msgBuffer.Put(p == player);
                msgBuffer.Put(p.Alive);
                if (player.NetConn.ConnectionState == ConnectionState.Connected)
                    player.NetConn.Send(msgBuffer, 2, DeliveryMethod.ReliableOrdered);
                msgBuffer = new NetDataWriter();
                msgBuffer.Put((byte)InfiniminerMessage.PlayerSetTeam);
                msgBuffer.Put((uint)p.ID);
                msgBuffer.Put((byte)p.Team);
                if (player.NetConn.ConnectionState == ConnectionState.Connected)
                    player.NetConn.Send(msgBuffer, 2, DeliveryMethod.ReliableOrdered);            }
            ...
            foreach (KeyValuePair<Vector3, Beacon> bPair in beaconList)
            {
                ...
                msgBuffer.Put((byte)InfiniminerMessage.SetBeacon);
                msgBuffer.Put(position);
                msgBuffer.Put(bPair.Value.ID);
                msgBuffer.Put((byte)bPair.Value.Team);
                if (player.NetConn.ConnectionState == ConnectionState.Connected)
                    player.NetConn.Send(msgBuffer, 2, DeliveryMethod.ReliableOrdered);            }
            ...
            msgBuffer.Put((byte)InfiniminerMessage.PlayerJoined);
            msgBuffer.Put((uint)player.ID);
            msgBuffer.Put(player.Handle);
            msgBuffer.Put(false);
            msgBuffer.Put(player.Alive);

            foreach (NetPeer netConn in playerList.Keys)
                if (netConn != player.NetConn && netConn.ConnectionState == ConnectionState.Connected)
                    netConn.Send(msgBuffer, 2, DeliveryMethod.ReliableOrdered);
            ...
            msgBuffer.Put((byte)InfiniminerMessage.ChatMessage);
            msgBuffer.Put((byte)ChatMessageType.SayAll);
            msgBuffer.Put(player.Handle + " HAS JOINED THE ADVENTURE!");
            foreach (var netConn in playerList.Keys)
                if (netConn.ConnectionState == ConnectionState.Connected)
                    netConn.Send(msgBuffer, 3, DeliveryMethod.ReliableOrdered);
        }
        ...
        {
            var msgBuffer = new NetDataWriter();
            msgBuffer.Put((byte)InfiniminerMessage.GameOver);
            msgBuffer.Put((byte)winningTeam);
            foreach (var netConn in playerList.Keys)
                if (netConn.ConnectionState == ConnectionState.Connected)
                    netConn.Send(msgBuffer, 0, DeliveryMethod.ReliableUnordered);     
        }
        ...
        {
            var msgBuffer = new NetDataWriter();
            msgBuffer.Put((byte)InfiniminerMessage.PlayerLeft);
            msgBuffer.Put((uint)player.ID);
            foreach (var netConn in playerList.Keys)
                if (netConn != player.NetConn && netConn.ConnectionState == ConnectionState.Connected)
                    netConn.Send(msgBuffer, 2, DeliveryMethod.ReliableOrdered);
            ...
            msgBuffer.Put((byte)InfiniminerMessage.ChatMessage);
            msgBuffer.Put((byte)ChatMessageType.SayAll);
            msgBuffer.Put(player.Handle + " " + reason);
            foreach (var netConn in playerList.Keys)
                if (netConn.ConnectionState == ConnectionState.Connected)
                    netConn.Send(msgBuffer, 3, DeliveryMethod.ReliableOrdered);
        }
        ...
        public void SendPlayerSetTeam(Player player)
        {
            ...
            msgBuffer.Put((byte)InfiniminerMessage.PlayerSetTeam);
            msgBuffer.Put((uint)player.ID);
            msgBuffer.Put((byte)player.Team);
            foreach (var netConn in playerList.Keys)
                if (netConn.ConnectionState == ConnectionState.Connected)
                    netConn.Send(msgBuffer, 2, DeliveryMethod.ReliableOrdered);
        }
        ...
        {
            var msgBuffer = new NetDataWriter();
            msgBuffer.Put((byte)InfiniminerMessage.PlayerDead);
            msgBuffer.Put((uint)player.ID);
            foreach (var netConn in playerList.Keys)
                if (netConn.ConnectionState == ConnectionState.Connected)
                    netConn.Send(msgBuffer, 2, DeliveryMethod.ReliableOrdered);
        ...
        {
            var msgBuffer = new NetDataWriter();
            msgBuffer.Put((byte)InfiniminerMessage.PlayerAlive);
            msgBuffer.Put((uint)player.ID);
            foreach (var netConn in playerList.Keys)
                if (netConn.ConnectionState == ConnectionState.Connected)
                    netConn.Send(msgBuffer, 2, DeliveryMethod.ReliableOrdered);
        ...
        {
            var msgBuffer = new NetDataWriter();
            msgBuffer.Put((byte)InfiniminerMessage.PlaySound);
            msgBuffer.Put((byte)sound);
            msgBuffer.Put(true);
            msgBuffer.Put(position);
            foreach (var netConn in playerList.Keys)
                if (netConn.ConnectionState == ConnectionState.Connected)
