...
namespace Infiniminer.States
{
    public class MainGameState : State
    {
        ...
        public override string OnUpdate(double gameTime)
        {
            ...
            _P.screenEffectCounter += gameTime;
            ...
            {
                _P.playerToolCooldown -= (float)gameTime;
                if (_P.playerToolCooldown <= 0)
                    _P.playerToolCooldown = 0;
            }
            
            // Digging like a freaking terrier! Now for everyone!
            if (_SM.Mouse.IsButtonDown(MouseButtons.Left) && !_P.playerDead && _P.playerToolCooldown == 0 && _P.playerTools[_P.playerToolSelected] == PlayerTools.Pickaxe)
            {
                _P.FirePickaxe();
            ...
            _SM.TextInputEnabled = _P.chatMode != ChatMessageType.None;

            return nextState;
        }

        private void UpdatePlayerPosition(double gameTime)
        {
            ...
            // Apply "gravity".
            _P.playerVelocity.Y += GRAVITY * (float)gameTime;
            ...
            _P.playerPosition += _P.playerVelocity * (float)gameTime;
            ...
            if (moveVector.X != 0 || moveVector.Z != 0)
            {
                ...
                moveVector *= MOVESPEED * (float)gameTime;
                if (movingOnRoad)
                    moveVector *= 2;
        ...
        private bool TryToMoveTo(Vector3 moveVector, double gameTime)
        {
        ...
        {
            _SM.RenderContext.SetCamera(_P.playerCamera);
            _P.skyplaneEngine.Render(_SM.RenderContext);
            _P.particleEngine.Render(_SM.RenderContext);
            _P.playerEngine.Render();
            _P.blockEngine.Render(_SM.RenderContext, gameTime);
            _P.playerEngine.RenderPlayerNames();
            _P.interfaceEngine.Render(_SM.RenderContext);
        ...
        void OnCharEntered(char c)
        {
            if ((int)c < 32 || (int)c > 126) //From space to tilde
                return; //Do nothing
            if (_P.chatMode != ChatMessageType.None)
            {
                ...
                if (diff.Milliseconds >= 2)
                    if (!(_SM.Keyboard.IsKeyDown(Keys.LeftControl) || _SM.Keyboard.IsKeyDown(Keys.RightControl)))
                    {
                        _P.chatEntryBuffer += c;
                    }
            }
        }
            ...
            switch (input)
            {
                ...
                case Buttons.Ping:
                {
                    var msgBuffer = new NetDataWriter();
                        ...
                        msgBuffer.Put(_P.playerMyId);
                        _P.netClient.Send(msgBuffer, DeliveryMethod.ReliableUnordered);
                }
                    break;
        ...
        {
            // Exit!
            if (key == Keys.Y && _SM.Keyboard.IsKeyDown(Keys.Escape))
            {
            ...
            // Pixelcide!
            if (key == Keys.K && _SM.Keyboard.IsKeyDown(Keys.Escape) && !_P.playerDead)
            {
            ...
            //Map saving!
            if ((_SM.Keyboard.IsKeyDown(Keys.LeftControl) || _SM.Keyboard.IsKeyDown(Keys.RightControl)) && key == Keys.S)
            {
                _P.SaveMap();
                return;
            }
            ...
            if (_P.chatMode != ChatMessageType.None)
            {
                if (_SM.Keyboard.IsKeyDown(Keys.LeftControl) || _SM.Keyboard.IsKeyDown(Keys.RightControl))
                {
                    if (key == Keys.V)
                    {
                        _P.chatEntryBuffer += _SM.GetClipboardText();
                        return;
                    }
                    else if (key == Keys.C)
                    {
                        _SM.SetClipboardText(_P.chatEntryBuffer);
                        return;
                    }
                    else if (key == Keys.X)
                    {
                ...
                if (key == Keys.Enter)
                {
                    ...
                    {
                        if (_P.netClient.ConnectionState == ConnectionState.Connected)
                        {
                            var msgBuffer = new NetDataWriter();
                            msgBuffer.Put((byte)InfiniminerMessage.ChatMessage);
                            msgBuffer.Put((byte)_P.chatMode);
                            msgBuffer.Put(_P.chatEntryBuffer);
                            _P.netClient.Send(msgBuffer, DeliveryMethod.ReliableOrdered);
