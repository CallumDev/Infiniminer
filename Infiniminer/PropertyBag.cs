...
namespace Infiniminer
{
    public class PropertyBag
    {
        ...
        public PropertyBag(InfiniminerGame gameInstance)
        {
            ...
            // Create a camera.
            playerCamera = new Camera(gameInstance.RenderContext);
            UpdateCamera();

            // Load sounds.
            if (!gameInstance.NoSound)
            {
                soundList[InfiniminerSound.DigDirt] = gameInstance.Content.LoadSound("sounds/dig-dirt.wav");
                soundList[InfiniminerSound.DigMetal] = gameInstance.Content.LoadSound("sounds/dig-metal.wav");
                soundList[InfiniminerSound.Ping] = gameInstance.Content.LoadSound("sounds/ping.wav");
                soundList[InfiniminerSound.ConstructionGun] = gameInstance.Content.LoadSound("sounds/build.wav");
                soundList[InfiniminerSound.Death] = gameInstance.Content.LoadSound("sounds/death.wav");
                soundList[InfiniminerSound.CashDeposit] = gameInstance.Content.LoadSound("sounds/cash.wav");
                soundList[InfiniminerSound.ClickHigh] = gameInstance.Content.LoadSound("sounds/click-loud.wav");
                soundList[InfiniminerSound.ClickLow] = gameInstance.Content.LoadSound("sounds/click-quiet.wav");
                soundList[InfiniminerSound.GroundHit] = gameInstance.Content.LoadSound("sounds/hitground.wav");
                soundList[InfiniminerSound.Teleporter] = gameInstance.Content.LoadSound("sounds/teleport.wav");
                soundList[InfiniminerSound.Jumpblock] = gameInstance.Content.LoadSound("sounds/jumpblock.wav");
                soundList[InfiniminerSound.Explosion] = gameInstance.Content.LoadSound("sounds/explosion.wav");
                soundList[InfiniminerSound.RadarHigh] = gameInstance.Content.LoadSound("sounds/radar-high.wav");
                soundList[InfiniminerSound.RadarLow] = gameInstance.Content.LoadSound("sounds/radar-low.wav");
                soundList[InfiniminerSound.RadarSwitch] = gameInstance.Content.LoadSound("sounds/switch.wav");
            }
        }
        ...
        {
            if (netClient.ConnectionState != ConnectionState.Connected)
                return;
            ...
            msgBuffer.Put(deathMessage);
            netClient.Send(msgBuffer, DeliveryMethod.ReliableUnordered);
        }
        ...
        public void addChatMessage(string chatString, ChatMessageType chatType, float timestamp)
        {
            ...
            {//each(string part in text){
                ...
                float incr = game.RenderContext.Renderer2D.MeasureString(Fonts.UiFont, part).X;
                curWidth += incr;
        ...
        public void DepositLoot()
        {
            ...
            var msgBuffer = new NetDataWriter();
            msgBuffer.Put((byte)InfiniminerMessage.DepositCash);
            netClient.Send(msgBuffer, DeliveryMethod.ReliableUnordered);
        }
        ...
        public void DepositOre()
        {
            ...
            var msgBuffer = new NetDataWriter();
            msgBuffer.Put((byte)InfiniminerMessage.DepositOre);
            netClient.Send(msgBuffer, DeliveryMethod.ReliableUnordered);
        }
        ...
        public void WithdrawOre()
        {
            ...
            var msgBuffer = new NetDataWriter();
            msgBuffer.Put((byte)InfiniminerMessage.WithdrawOre);
            netClient.Send(msgBuffer, DeliveryMethod.ReliableUnordered);
        }
        ...
        public void SetPlayerClass(PlayerClass playerClass)
        {
            if (this.playerClass != playerClass)
            {
                ...
                var msgBuffer = new NetDataWriter();
                msgBuffer.Put((byte)InfiniminerMessage.SelectClass);
                msgBuffer.Put((byte)playerClass);
                netClient.Send(msgBuffer, DeliveryMethod.ReliableUnordered);
        ...
        public void FireRadar()
        {
            ...
            var msgBuffer = new NetDataWriter();
            msgBuffer.Put((byte)InfiniminerMessage.UseTool);
            msgBuffer.Put(playerPosition);
            msgBuffer.Put(playerCamera.GetLookVector());
            msgBuffer.Put((byte)PlayerTools.ProspectingRadar);
            msgBuffer.Put((byte)BlockType.None);
            netClient.Send(msgBuffer, DeliveryMethod.ReliableUnordered);
        }
        ...
        public void FirePickaxe()
        {
            ...
            msgBuffer.Put(playerPosition);
            msgBuffer.Put(playerCamera.GetLookVector());
            msgBuffer.Put((byte)PlayerTools.Pickaxe);
            msgBuffer.Put((byte)BlockType.None);
            netClient.Send(msgBuffer, DeliveryMethod.ReliableUnordered);
        }
        ...
        public void FireConstructionGun(BlockType blockType, bool alternate)
        {
            ...
            msgBuffer.Put(playerPosition);
            msgBuffer.Put(playerCamera.GetLookVector());
            msgBuffer.Put((byte)PlayerTools.ConstructionGun);
        ...
        public void FireDeconstructionGun()
        {
            ...
            msgBuffer.Put(playerPosition);
            msgBuffer.Put(playerCamera.GetLookVector());
            msgBuffer.Put((byte)PlayerTools.DeconstructionGun);
            msgBuffer.Put((byte)BlockType.None);
            netClient.Send(msgBuffer, DeliveryMethod.ReliableUnordered);
        }
        ...
        public void FireDetonator()
        {
            ...
            msgBuffer.Put(playerPosition);
            msgBuffer.Put(playerCamera.GetLookVector());
            msgBuffer.Put((byte)PlayerTools.Detonator);
            msgBuffer.Put((byte)BlockType.None);
            netClient.Send(msgBuffer, DeliveryMethod.ReliableUnordered);
        }
        ...
        public void ReadRadar(ref float distanceReading, ref float valueReading)
        {
            ...
            for (int i = -3; i <= 3; i++)
                for (int j = -3; j <= 3; j++)
                {
                    Matrix4x4 rotation = Matrix4x4.CreateRotationX((float)(i * Math.PI / 128)) * Matrix4x4.CreateRotationY((float)(j * Math.PI / 128));
        ...
        public void SendPlayerUpdate()
        {
            ...
            msgBuffer.Put(playerPosition);
            msgBuffer.Put(playerCamera.GetLookVector());
            msgBuffer.Put((byte)playerTools[playerToolSelected]);
            msgBuffer.Put(playerToolCooldown > 0.001f);
            netClient.Send(msgBuffer, DeliveryMethod.Sequenced);
        }
    }
}
