...
namespace Infiniminer
{
    public class PlayerEngine
    {
        ...
        public PlayerEngine(InfiniminerGame gameInstance)
        {
            ...
            vbo = new VertexBuffer(gameInstance.RenderContext, typeof(VertexPositionTexture), 6, true);
        }
        ...
        public void Update(double elapsed)
        {
            ...
            {
                p.StepInterpolation(gameInstance.TotalTime);

                p.Ping -= (float) elapsed;
                if (p.Ping < 0)
                    p.Ping = 0;

                p.TimeIdle += (float) elapsed;
                if (p.TimeIdle > 0.5f)
                    p.IdleAnimation = true;
                p.SpriteModel.Update(elapsed);
            }
        }
        ...
        public void Render()
        {
            ...
            foreach (Player p in _P.playerList.Values)
            {
                ...
                {
                    p.SpriteModel.Draw(gameInstance.RenderContext,
                        vbo,
            ...
            foreach (Player p in _P.playerList.Values)
            {
                if (p.Alive && p.ID != _P.playerMyId)
                {
                    ...
                    if (continueDraw)//p.ID != _P.playerMyId && p.Team == _P.playerTeam)
                    {
                        ...
                        p.SpriteModel.DrawText(_P.playerCamera, gameInstance.RenderContext,
                                               p.Position - Vector3.UnitY * 1.5f,
