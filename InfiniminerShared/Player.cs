...
namespace Infiniminer
{
    ...
    public class Player
    {
        ...
        public Color4 redTeam = new Color4();
        public Color4 blueTeam = new Color4();
        ...
        public Player(NetPeer netConn, Game gameInstance)
        {
            ...
            if (netConn != null)
                this.IP = netConn.Address.ToString();
        ...
        private void UpdateSpriteTexture()
        {
            ...
            textureName += ".png";
            Texture2D orig = gameInstance.GetService<ContentManager>().LoadTexture(textureName);
            if (AltColours)// && ((team == PlayerTeam.Blue && blueTeam != Defines.IM_BLUE) || (team == PlayerTeam.Red && redTeam != Defines.IM_RED)))
            {
                uint[] data = new uint[orig.Width * orig.Height];
                orig.GetData<uint>(data);
                Texture2D temp = new Texture2D(gameInstance.RenderContext, orig.Width,orig.Height);
                temp.SetData<uint>(data);
